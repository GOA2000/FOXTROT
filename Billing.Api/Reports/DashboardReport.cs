...
namespace Billing.Api.Reports
{
    ...
    public class DashboardReport : BaseController
    {
        ...
        public DashboardModel Report(int month = 0)
        {
            ...
            result.Agent = Identity.CurrentUser.Name;
            ...
            result.Agents = invoices.GroupBy(x => x.Agent.Name)
                                    ...
                                    .ToList();
            result.Regions = new List<DashboardModel.Monthly>();
            for (int i = 0; i < 8; i++) result.Regions.Add(new DashboardModel.Monthly() { Sales = 0 });
            result.Regions = invoices.GroupBy(x => x.Customer.Town.Region.ToString())
                                     .Select(x => new DashboardModel.Monthly() { Label = x.Key, Sales = x.Sum(y => y.SubTotal) })
                                     .ToList();

            /*result.Regions = new List<DashboardModel.Monthly>();
            for (int i = 0; i < 8; i++) result.Regions.Add(new DashboardModel.Monthly() { Label = i.ToString(), Sales = 0 });
            var regionSale = invoices.GroupBy(x => x.Customer.Town.Region)
                                     .Select(x => new { index = (int)x.Key, Label = x.Key, value = x.Sum(y => y.SubTotal) })
                                     .ToList();
            foreach (var region in regionSale)
            {
                result.Regions[region.index - 1].Sales = region.value;
            }*/
